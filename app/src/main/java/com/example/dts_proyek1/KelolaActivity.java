...
public class KelolaActivity extends AppCompatActivity implements View.OnClickListener {
    public static final int REQUEST_CODE_STORAGE = 100;
    int eventID = 0;
    EditText txtFileName, txtCatatan;
    Button btnSimpan;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setSubtitle("Tambah Catatan");
...
//            getSupportActionBar().setTitle("Tambah Catatan");
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btnSimpan:
                eventID = 2;
                if (!tempCatatan.equals(txtCatatan.getText().toString())) {
                    if (Build.VERSION.SDK_INT >= 23) {
                        if (periksaIzinPenyimpanan()) {
    ...
    void bacaFile() {
        ...
        if (file.exists()) {
            ...
            txtCatatan.setText(text.toString());
        }
    }
    ...
    void buatDanUbah() {
        ...
        if (parent.exists()) {
            ...
            try {
                ...
                streamWriter.append(txtCatatan.getText());
                streamWriter.flush();
                streamWriter.close();
                outputStream.flush();
                outputStream.close();
            ...
            parent.mkdir();
            File file = new File(path, txtFileName.getText().toString()+".txt");
            FileOutputStream outputStream = null;
            try {
                file.createNewFile();
                outputStream = new FileOutputStream(file, false);
                outputStream.write(txtCatatan.getText().toString().getBytes());
                outputStream.flush();
                outputStream.close();
            } catch (Exception e) {
                e.printStackTrace();
